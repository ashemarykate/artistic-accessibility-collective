...
export default function PublicDirectory() {
  ...
  if (loading) {
    return (
      <div className="loading-screen" aria-live="polite" aria-label="Loading directory">
    ...
    <main>
      <header className="site-header">
        ...
        <nav className="site-nav" aria-label="Main navigation">
          <Link href="/submit" className="nav-link">Join</Link>
      ...
      <div className="page-container-wide" style={{ paddingTop: '2.5rem' }}>
        <div style={{ marginBottom: '2rem' }}>
          ...
          <p className="font-accent-italic" style={{ color: 'rgba(255,255,255,0.75)', fontSize: '1rem' }}>
            {profiles.length} professional{profiles.length !== 1 ? 's' : ''} in our directory
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '2rem' }}>
          <div className="form-group" style={{ flex: '1 1 260px' }}>
            <label htmlFor="search" className="sr-only">Search professionals</label>
            <input
              id="search"
              type="search"
        ...
        {filtered.length === 0 ? (
          <div className="content-card" style={{ textAlign: 'center', padding: '3rem' }}>
            <p style={{ color: 'var(--color-text-muted)' }}>
              {profiles.length === 0
                ? 'No profiles are listed yet. Check back soon!'
        ...
        <div style={{ textAlign: 'center', marginTop: '3rem' }}>
          <p style={{ color: 'rgba(255,255,255,0.65)', marginBottom: '1rem', fontSize: '0.9375rem' }}>
            Are you an accessibility professional?
          </p>
          <Link href="/submit" className="btn btn-on-blue">
            Join the Registry
          </Link>
        </div>
      </div>
