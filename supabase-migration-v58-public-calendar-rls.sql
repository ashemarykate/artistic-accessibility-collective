-- ─────────────────────────────────────────────────────────────────────────────
-- Migration v58 (calendar) · Fix the public calendar for logged-out visitors
--
-- ALREADY APPLIED on 2026-09-09, and verified: a logged-out visitor can read
-- the calendar again. Kept as the record of what was changed. Safe to re-run.
--
-- NUMBERING NOTE. Two Claude sessions were working in this folder on the same
-- day and both wrote a file called supabase-migration-v58.sql: this one, and
-- the 2006 wall's "give the seeded posts real notes". Both have been run. This
-- copy carries a longer filename so neither is lost. If you renumber anything
-- later, the wall one is the better candidate to become v60, because this one
-- is the file that was sent and run under the name v58.
-- ─────────────────────────────────────────────────────────────────────────────
--
